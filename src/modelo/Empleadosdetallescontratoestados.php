<?php 
/**
 *
 * @author yalfonso
 *
 */
class Empleadosdetallescontratoestados extends Clsdatos { 

	private $id = 0; 
	private $nombre = ""; 

	/**
	 * Obtiene el identificador del estado del detalle del contrato.
	 *
	 * @return mixed El valor del identificador.
	 */
	public function getId (){ 
		return $this->id;
	} 

	/**
	 * Establece el identificador del estado del detalle del contrato.
	 *
	 * @param mixed $vl Identificador a asignar.
	 * @return void
	 */
	public function setId ( $vl ){ 
		$this->id = $vl;
	} 

	/**
	 * Obtiene el nombre del estado del detalle del contrato.
	 *
	 * @return mixed El valor del nombre.
	 */
	public function getNombre (){ 
		return $this->nombre;
	} 

	/**
	 * Establece el nombre del estado del detalle del contrato.
	 *
	 * @param mixed $vl Nombre a asignar.
	 * @return void
	 */
	public function setNombre ( $vl ){ 
		$this->nombre = $vl;
	} 

} 
?>
