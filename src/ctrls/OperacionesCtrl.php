...
HTML;
	...
	// Gestor Documental
	const CFG_GA_ACTIVAR = 'cfggaactivar';
	const CFG_GA_IP = 'cfggaip';
	const CFG_GA_LOGIN = 'cfggalogin';
	const CFG_GA_CLAVE = 'cfggaclave';
	const CFG_GA_JSON_TIPODOC = 'cfggajsontipodoc';
	const CFG_GA_ELIMINARFUENTE = 'cfggaeliminarfuente';
...
EOD;
	...
	public static function flujositems_Obtener ( $d ){
	    ...
	    try {
	        ...
	        http_response_code( IndexCtrl::ERR_COD_MSJ_ERR_COMUN );
	        throw new \Exception( 'flujositems_Obtener: ' . $e->getMessage() , IndexCtrl::ERR_COD_MSJ_ERR_COMUN);
	    }
	    ...
	    if ( count( $objFlujo ) > 0 ) {
	        ...
	        if ( isset( $json[ 'id' ] ) ) {
	            ...
	            if ( $objPaquetes[0]['paquetesestados_id'] == 1 ) {
	                $modPck = array('id' => $tpls_id, 'nombre' => $json['nombre'], 'mesaplica' => $json['mesaplica'] );
	                try {
	                    self::paquetes_Modificar( $modPck );
	                } catch (Exception $e) {
	                    throw new Exception( 'flujos_Helper_Agregar - flujos_Modificar: ' . $e->getMessage(), $e->getCode() );
	                }
	            }
	...
	public static function paquetes_Helper_MoverAdmin( $d ){
	    ...
	    if( $debesinc ){
	        if ( $_CFG_GA_ACTIVAR ) {
	            $essinc = self::paquetes_Obtener( [ 'id' => $idMod ] );
	            $radicado = "";

	            $parasinc = [];
	            foreach ( $essinc as $kEss ) {
	                
	                // siempre obtenermos el radicado
	                if ( $kEss['radicado'] == "" ) {
	                    $dataem = base64_encode( json_encode( [ "empleados_id" => $kEss['empleados_id'] ] ) );
	                    
	                    try {
	                        $rRadi = self::gestordocumental_Crear_Radicado( ['data' => $dataem ] );
	                    } catch (Exception $e) {
	                        http_response_code( $e->getCode() );
	                        return self::retorno([], $e->getCode(), "paquetes_Helper_MoverAdmin - gestordocumental_Crear_Radicado: " . $e->getMessage());
	                    }
	                    
	                    $radJson = $rRadi['result'];
	                    if ( isset( $radJson['estado']) ) {
	                        $radicado = $radJson['estado'];
	                    }
	                    else{
	                        http_response_code( IndexCtrl::ERR_COD_CAMPO_OBLIGATORIO );
	                        return self::retorno([], IndexCtrl::ERR_COD_CAMPO_OBLIGATORIO, "Respuesta Gestor documental obteniendo radicado ( " . print_r( $radJson , true ) . ")" );
	                    }
	                    
	                    self::paquetes_Modificar( [ 'id' => $idMod , "radicado" => $radicado ] );
	                }
	                else {
	                ...
	                if( $kEss[ 'flujositems_sincronizar' ] == 1 || ( isset( $modCfg['paquetesestados_id'] ) && $modCfg['paquetesestados_id'] == 4) ){
	                    ...
	                    $pkreq = self::paquetesrequ_Obtener([ 'w_paquetes_id' => $kEss['id'] ]);
	                    
	                    $parasinc[ self::GESTORDOC_ARRAYID_GENERADOS ] = self::paquetes_Helper_ObtenerArchivosGenerados( [ 'archivos' => $docs ] );
	                    $parasinc[ self::GESTORDOC_ARRAYID_FORMULARIOS ] = self::paquetes_Helper_ObtenerArchivosDeFormularios( [ 'archivos' => $pkreq , 'formulario' => true ] );
	                    $parasinc[ self::GESTORDOC_ARRAYID_CAMPOS ] = self::paquetes_Helper_ObtenerArchivosDeFormularios( [ 'archivos' => $pkreq , 'formulario' => false ] );
	                    $parasinc[ self::GESTORDOC_ARRAYID_OBLIGACIONES ] = self::paquetes_Helper_ObtenerArchivosObligaciones( [ 'archivos' => $pkreq ] );
	                    
	                    $resSinc = self::gestordocumental_ProcesarMasivo( [ 'archivos' => $parasinc, 'radicado' => $radicado ] );
	                    //die( "resSinc:\n" . print_r( $resSinc, true ) );
	                    try {
	                        self::sincronizacion_Helper_Agregar( [ "archivos" => $resSinc ] );
	                    } catch (Exception $e) {
	                        http_response_code( $e->getCode() );
	                        return self::retorno([], $e->getCode(), "paquetes_Helper_MoverAdmin - sincronizacion_Helper_Agregar: " . $e->getMessage());
	                    }
	                }
	            }
	...
	// TODO: Tarea 169 - Crear una funcion que obtenga las los archivos de la obligaciones
	private static function paquetes_Helper_ObtenerArchivosObligaciones( $d ){
	    $pack = $d[ 'archivos' ];
	    $res = [];
	    
	    foreach ($pack as $kPk ) {
	        if ( $kPk['paquetereqtipos_id'] == 7 ) {
	            $fls = self::empleadosobjetivoslog_Obtener([
	                "w_paquetesrequ_id" => $kPk['id']
	            ]);
	            foreach ( $fls as $kFls ) {
	                if( strlen( $kFls['archivos'] ) > 0 ){
    	                $fl = "/" . ltrim( $kFls['archivos'] , "/") ;
    	                $desc = $kFls['empleadosobjetivos_descripcion'];
    	                if( strlen( $desc ) >= 480 ){
    	                    $desc = mb_substr($desc, 0, 450, "UTF-8");
    	                }
	...
	// TODO: Tarea 168 - Crear una funcion que obtenga las los archivos generados
	private static function paquetes_Helper_ObtenerArchivosGenerados( $d ){
	    $pack = $d[ 'archivos' ];
	    $res = [];
	    foreach ( $pack as $kPk ) {
	        $fl = "/" . ltrim( $kPk['bs'] , "/") ;
	...
	// TODO: Tarea 167 - Crear una funcion que obtenga las respuestas de los formularios y capture los archivos cargados
	private static function paquetes_Helper_ObtenerArchivosDeFormularios( $d ){
        $pack = $d[ 'archivos' ];
        $formulario = $d[ 'formulario' ];
        
        $res = [];
        foreach ( $pack as $kPk ) {
            if( $kPk['paquetereqtipos_id'] == 4 ){
                if( !$formulario ){
                    $res[ $kPk['id'] ] = [
                        "archivo" => "/" . ltrim( $kPk['valor'] , "/"),
                        "desc" => $kPk['ref_label']
                    ];
                }
            }
                ...
                if( $formulario ){
                    $json = json_decode( $kPk['valor'], true );
                    if( is_array( $json ) ){
                        foreach ( $json as $jsDt ) {
                            if ( $jsDt['file'] ) {
                                $res[ $jsDt['field'] ] = [
                                    "archivo" => "/" . ltrim( $jsDt['value'] , "/"),
                                    "desc" => $jsDt['label']
                                ];
                            }
                        }
	...
	*/
	...
	public static function reflista_Obtener ( $d ){
	    ...
	    if ( isset( $r['err_info'] )) {
	        http_response_code( IndexCtrl::ERR_COD_MSJ_ERR_COMUN );
	        throw new \Exception( 'reflista_Obtener: ' . $r['err_info'] , IndexCtrl::ERR_COD_MSJ_ERR_COMUN);
	    }
	...
	*/
	...
	// Sincronizacion INI
	// TODO: Tarea 170 - Crear una función que permita agregar registros en la tabla Sincronizacion
	/**
	 * Agrega un nuevo registro a la tabla "sincronizacion".
	 *
	 * @param array $d Datos para crear el nuevo registro sincronizado.
	 * @return mixed Identificador del registro creado.
	 * @throws \Exception Si ocurre un error durante la creación.
	 */
	public static function sincronizacion_Agregar( $d ) {
	    date_default_timezone_set('America/Bogota');
	    
	    $o = new Sincronizacion();
	    if (isset( $d['tabla'] ) ) {
	        $o->setTabla( $d['tabla'] );
	    }
	    if (isset( $d['idref'] ) ) {
	    ...
	    if (isset( $d['archivo'] ) ) {
	        $o->setArchivo( $d['archivo'] );
	    }
	    if (isset( $d['gestor'] ) ) {
	        $o->setGestor( $d['gestor'] );
	    }
	    if (isset( $d['usuarios'] ) ) {
	        $o->setUsuarios( $d['usuarios'] );
	    }
	    ...
	    if (isset( $d['sincronizacionestados_id'] ) ) {
	        $o->setSincronizacionestados_id( $d['sincronizacionestados_id'] );
	    }
	    if (isset( $d['mensaje'] ) ) {
	...
	// TODO: Tarea 171 - Crear una función helper que permite decidir acciones sobre la tabla Sincronizacion
	/**
	 * Controlar ingresos o modificaciones a la tabla "sincronizacion".
	 *
	 * @param array $d Datos para crear el nuevo registro sincronizado.
	 * @return mixed Identificador del registro creado.
	 * @throws \Exception Si ocurre un error durante la creación.
	 */
	public static function sincronizacion_Helper_Agregar( $d ) {
	    ...
	    foreach ( $archivos as $kFls => $vFls ) {
	        foreach ($vFls as $kSincs) {
	            $result = $kSincs['result'];
	            
	            $eId = $kSincs[ 'error' ];
	            
	            $sincronizacionestados_id = 1;
	            $mensaje = "";
	            if( $eId > 0 ){
	                $sincronizacionestados_id = 3;
	                $mensaje = $kSincs['msj'];
	            }
	            $regs[ md5( $result['original'] ) ] = [
	                "tabla" => $kFls,
	                "idref" => "0",
	                "archivo" => $result['original'],
	                "w_archivo" => $result['original'],
	                "gestor" => $result['ruta_completa'],
	                "usuarios" => trim( (string) $usu->getNombres() . ' ' . $usu->getApellidos() ),
	                "fecha" => date("Y-m-d H:i:s"),
	                "sincronizacionestados_id" => $sincronizacionestados_id,
	                "mensaje" => $mensaje
	            ];
	        }
	    }
	    
	    $buscar = array_column( $regs , 'archivo' );
	    $existentes = self::sincronizacion_Obtener( [ 'w_archivos' => $buscar ] );
	    
	    foreach ( $regs as $kReg => $vReg ) {
	        $actualizar = false;
	        foreach ( $existentes as $vEx ) {
	            if( md5( $vEx['archivo'] ) == $kReg ){
	                $actualizar = true;
	                if ( $vEx['sincronizacionestados_id'] == 3 ) {
	                    self::sincronizacion_Modificar( $vReg );
	                }
	        ...
	        if ( !$actualizar ) {
	            self::sincronizacion_Agregar( $vReg );
	        }
	    }
	    
	    return $existentes;
	}
	// TODO: Tarea 172 - Crear una función para obtener los registros de Sincronizacion
	/**
	 * Obtiene los registros de "sincronizacion".
	 *
	 * @param array $d Datos para obtener registros.
	 * @return mixed Un arreglo con todos los registros.
	 * @throws \Exception Si ocurre un error durante la creación.
	 */
	public static function sincronizacion_Obtener ( $d ) {
	    try {
	        self::authRequ();
	    } catch (\Exception $e) {
	        http_response_code( IndexCtrl::ERR_COD_SESION_INACTIVA );
	    ...
	    $vr  = "sincr.`id`, sincr.`tabla`, sincr.`idref`, sincr.`archivo`, ";
	    $vr .= "SUBSTRING_INDEX(SUBSTRING_INDEX( TRIM(LEADING '/' FROM sincr.`archivo` ), '/', 4),'/', -1) as empleados_documento, ";
	    $vr .= "empl.id as empleados_id, sincr.`gestor`, sincr.`usuarios`, sincr.`fecha`, ";
	    $vr .= "sincr.`sincronizacionestados_id`, sincr.`mensaje` ";
	    
	    $tb  = '`sincronizacion` as sincr ';
	    
	    $jn  = "LEFT JOIN empleados as empl on empl.documento = SUBSTRING_INDEX(SUBSTRING_INDEX( TRIM(LEADING '/' FROM sincr.`archivo` ), '/', 4),'/', -1) ";
	    ...
	    if( isset( $d['w_archivo'] ) ){
	        $wh[] = "sincr.`archivo` = ?";
	        $pr[] = $d['w_archivo'];
	    }
	    
	    if( isset( $d['w_archivos'] ) ){
	        $ids = $d['w_archivos'];
	        $placeholders = implode(',', array_fill(0, count($ids), '?'));
	        $wh[] = "sincr.`archivo` IN (" . $placeholders . ")";
	    ...
	    if( isset( $d['w_sincronizacionestados_id'] ) ){
            $wh[] = "sincr.`sincronizacionestados_id` = ?";
            $pr[] = $d['w_sincronizacionestados_id'];
        }
	...
	public static function sincronizacion_Modificar($d) {
	    date_default_timezone_set('America/Bogota');
	    ...
	    try {
	        ...
	        http_response_code( IndexCtrl::ERR_COD_SESION_INACTIVA );
	        throw new \Exception( "sincronizacion_Modificar: " . $e->getMessage(), IndexCtrl::ERR_COD_SESION_INACTIVA );
	    }
	    ...
	    if ( isset( $d['archivo'] ) ) {
	        $aSt['archivo'] = $d['archivo'] ;
	    }
	    if ( isset( $d['gestor'] ) ) {
	        $aSt['gestor'] = $d['gestor'] ;
	    }
	    
	    $aSt['usuarios'] = trim( $usu->getNombres() . " " . $usu->getApellidos() );
	    $aSt['fecha'] = date("Y-m-d H:i:s");
	    
	    if ( isset( $d['sincronizacionestados_id'] ) ) {
	        $aSt['sincronizacionestados_id'] = $d['sincronizacionestados_id'] ;
	    }
	    if ( isset( $d['mensaje'] ) ) {
	    ...
	    if ( $wh == '' ) {
	        http_response_code( IndexCtrl::ERR_COD_CAMPO_OBLIGATORIO );
	        throw new Exception( 'sincronizacion_Modificar: Debe indicar un filtro para actualizar', IndexCtrl::ERR_COD_CAMPO_OBLIGATORIO );
	    }
	    ...
	    try {
	        ...
	        http_response_code( IndexCtrl::ERR_COD_ACTUALIZACION_SQL );
	        throw new \Exception( 'sincronizacion_Modificar: ' . $th->getMessage() , IndexCtrl::ERR_COD_ACTUALIZACION_SQL );
	    }
	...
	const GESTORDOC_ARRAYID_GENERADOS = 'generados';
	const GESTORDOC_ARRAYID_FORMULARIOS = 'formularios';
	const GESTORDOC_ARRAYID_CAMPOS= 'campos';
	const GESTORDOC_ARRAYID_OBLIGACIONES = 'obligaciones';
	...
	public static function gestordocumental_Obtener_Tipo( $d ){
	    $empleados = $d['empleados'];
	    $tipo = $empleados['tipodoc_id'];
	    $r = [
	        "tipo" => "1",
	        "persona" => "N"
	    ];
	...
	public static function gestordocumental_Obtener_Tipodoc( $d ){
	    $json = $d['json'] ;
	    $tipodoc_id = $d['tipodoc_id'];
	    
	    $r = 13;
	    foreach ( $json as $kJson ) {
	        if ( $kJson['idgestorfinanciero'] == $tipodoc_id ) {
	            $r = $kJson['idorfeo'];
	        }
	    }
	...
	public static function gestordocumental_Crear_Radicado( $d ){
	    ...
	    if( $_CFG_GA_ACTIVAR ){
	        $_CFG_GA_IP = isset( $cfg[ self::CFG_GA_IP ] ) ? $cfg[ self::CFG_GA_IP ]["val"] : "";
	        $_CFG_GA_CLAVE = isset( $cfg[ self::CFG_GA_CLAVE ] ) ? $cfg[ self::CFG_GA_CLAVE ]["val"] : "";
	        $_CFG_GA_JSON_TIPODOC = isset( $cfg[ self::CFG_GA_JSON_TIPODOC ] ) ? base64_decode( $cfg[ self::CFG_GA_JSON_TIPODOC ]["val"] ) : "[]";
	        
	        $b64 = base64_decode( $d['data'] );
	        $json = json_decode( $b64, true ) ;
	        
	        $usr = self::empleados_Obtener( [ 'id' => $json['empleados_id'] ] );
	        foreach ( $usr as $kUsr ) {
	            $curl = curl_init();
	            
	            $json_tipodoc = json_decode( $_CFG_GA_JSON_TIPODOC , true );
	            $documento = $kUsr['documento'];
	            $tipo = self::gestordocumental_Obtener_Tipo( [ 'empleados' => $kUsr ] );
	            $tipodoc = self::gestordocumental_Obtener_Tipodoc( [ 
	                'tipodoc_id' => $kUsr['tipodoc_id'] , 
	                'json' => $json_tipodoc
	            ] );
	            ...
	            $poststr = '{
	                ...
	                "tipodoc": ' . $tipodoc . ',
	                "cedulaNatural": "' . $documento . '",
	                "nombres": "' . $kUsr['nombres'] . '",
	                "apellido1": "' . $kUsr['apellidos'] . '",
	                "apellido2": "",
	                "direccion": "' . $kUsr['direccion'] . '",
	                "telefono": "-",
	                "email": "' . $kUsr['mail'] . '",
	                "departamento": "11",
	                "municipio": 1,
	            ...
	            $options = array(
	                ...
	                CURLOPT_HTTPHEADER => array(
	                    'Authorization: Bearer ' . base64_decode( $_CFG_GA_CLAVE) ,
	                    'Content-Type: application/json'
	                ),
	            );
	            
	            if (stripos($_CFG_GA_IP, 'https://') === 0) {
	                $pathCA = dirname(dirname(dirname(__FILE__))) . "/" . Config::CARPETA_REPOSITORIOS . '/corp/';
	                $cafl = $pathCA . 'cacert.pem';
	                
	                if( file_exists( $cafl ) ){
	                    $options[CURLOPT_SSL_VERIFYPEER] = false;
	                    $options[CURLOPT_SSL_VERIFYHOST] = false;
	                    
    	                //$options[CURLOPT_CAINFO] = $cafl;
    	                //$options[ CURLOPT_CAPATH ] = $pathCA;
	            ...
	            if( isset( $res['error'] ) ){
	                http_response_code( IndexCtrl::ERR_COD_MSJ_ERR_COMUN );
	                return self::retorno( [] , IndexCtrl::ERR_COD_MSJ_ERR_COMUN, $res['error'] );
	            }
	            else {
	                if ( is_array( $res ) ) {
	                    if ( count( $res ) > 0  ) {
	                        return self::retorno( $res , 0, "" );
	                    }
	                    else{
	                        http_response_code( IndexCtrl::ERR_COD_RESPUESTA_SQL_VACIA );
	                        return self::retorno( [] , IndexCtrl::ERR_COD_RESPUESTA_SQL_VACIA, "Gestor documental no retorna valores" );
	                    }
	                }
	                else {
	                    http_response_code( IndexCtrl::ERR_COD_CAMPO_OBLIGATORIO );
	                    return self::retorno( [] , IndexCtrl::ERR_COD_CAMPO_OBLIGATORIO, "Gestor documental responde: " . $res );
	                }
	                
	            }
	            
	            return self::retorno( [] , IndexCtrl::ERR_COD_CAMPO_OBLIGATORIO, "Gestor documental responde: Sin datos" );
	        }
	        
	    }
	    else {
	        http_response_code( IndexCtrl::ERR_COD_CAMPO_OBLIGATORIO );
	        throw new Exception( 'Debe activar gestor documental', IndexCtrl::ERR_COD_CAMPO_OBLIGATORIO );
	    }
	    
	}
	
	public static function gestordocumental_ProcesarMasivo ( $d ){
	    $cfg = self::LeerConfigCorp();
	    $_CFG_GA_ELIMINARFUENTE = filter_var( isset( $cfg[ OperacionesCtrl::CFG_GA_ELIMINARFUENTE ] ) ? $cfg[ OperacionesCtrl::CFG_GA_ELIMINARFUENTE ]["val"] : false , FILTER_VALIDATE_BOOLEAN);
	    ...
	    foreach ( $pack as $kPk => $vPk ) {
	        foreach ( $vPk as $kData ) {
	            $cfg = $kData;
	            $cfg['radicado'] = $d['radicado'];
	            $b64Cfg = base64_encode( json_encode( $cfg ) );
	            
	            $sinc = [];
	            try {
	                $sinc = self::gestordocumental_Crear_Anexo( [ 'data' => $b64Cfg ] );
	                
	                if ( $err_rad ) {
	                    $sinc['error'] = IndexCtrl::ERR_COD_CAMPO_OBLIGATORIO;
	                }
	                
	                if( $sinc['error'] > 0 ){
	                    $msj = $sinc['msj'];
	                    if( $err_rad ){
	                        $msj = "No obtuvo un radicado y no se sincroniz&oacute; el archivo";
	                    }
	                    ...
	                    $rData = [  
	                        ...
	                        'archivo' => '',
	                        'ruta_completa' => '',
	                        'original' => $kData['archivo']
	                    ];
	                    $res[ $kPk ][] = self::retorno( $rData , IndexCtrl::ERR_COD_CAMPO_OBLIGATORIO, $msj );
	                }
	                else {
	                    $file = ltrim( $kData['archivo'] , "/");
	                    $sinc['result']['original'] = "/" . $file;
	                    $res[ $kPk ][] = $sinc;
	                    
	                    if ( $_CFG_GA_ELIMINARFUENTE ) {
	                        if ( file_exists( $kData['archivo'] ) ) {
	                            $ptfile = dirname(dirname(dirname( __FILE__ ))) . DIRECTORY_SEPARATOR . $file;
	                            unlink( $ptfile );
	                        }
	                    }
	                ...
	                $res[ $kPk ][] = [ 'archivo' => $kData['archivo'], 'err' => $e->getMessage() ];
	            }
	...
	public static function gestordocumental_Crear_Anexo( $d ){
	    ...
	    if( $_CFG_GA_ACTIVAR ){
	        $_CFG_GA_IP = isset( $cfg[ self::CFG_GA_IP ] ) ? $cfg[ self::CFG_GA_IP ]["val"] : "";
	        $_CFG_GA_LOGIN = isset( $cfg[ self::CFG_GA_LOGIN ] ) ? $cfg[ self::CFG_GA_LOGIN ]["val"] : "";
	        $_CFG_GA_CLAVE = isset( $cfg[ self::CFG_GA_CLAVE ] ) ? $cfg[ self::CFG_GA_CLAVE ]["val"] : "";
	        ...
	        $ptfile = dirname(dirname(dirname( __FILE__ ))) . DIRECTORY_SEPARATOR . ltrim( $file , "/");
	        if (!is_readable($ptfile)) { 
	            return self::retorno( [] , IndexCtrl::ERR_COD_RESPUESTA_SQL_VACIA, 'El archivo "' . $ptfile . '" no puede leerse' );
	        }
	        $fileContent = file_get_contents( $ptfile ); 
	        ...
	        if( isset( $res['error'] ) ){
	            ...
	            http_response_code( IndexCtrl::ERR_COD_MSJ_ERR_COMUN );
	            return self::retorno( [] , IndexCtrl::ERR_COD_MSJ_ERR_COMUN, $res['error'] . " (" . $jsonerr . ")" );
	        }
	        else {
	            return self::retorno( $res , 0, "" );
	        }
